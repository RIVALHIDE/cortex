...
class TranslationValidator:
    ...
    def _validate_file(self, filepath: Path) -> None:
        ...
        if missing_keys:
            self.errors.append(
                f"{lang_code}: Missing {len(missing_keys)} key(s): {missing_keys}"
            )
        ...
        if extra_keys:
            self.warnings.append(
                f"{lang_code}: Has {len(extra_keys)} extra key(s): {extra_keys}"
            )
    ...
    def _check_placeholders(
        ...
        if missing:
            self.warnings.append(
                f"{lang_code}/{key}: Missing placeholder(s): {missing}"
            )
        ...
        if extra:
            self.warnings.append(
                f"{lang_code}/{key}: Extra placeholder(s): {extra}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate Cortex Linux translation files"
    )
    parser.add_argument(
        "--strict",
        action="store_true",
