...
class LanguageManager:
    ...
    def detect_language(self, cli_arg: Optional[str] = None) -> str:
        ...
        if cli_arg and self.is_supported(cli_arg):
            ...
            logger.warning(
                f"Language '{cli_arg}' not supported. Falling back to detection."
            )
        ...
        if env_lang and self.is_supported(env_lang):
            ...
            logger.warning(
                f"Language '{env_lang}' in CORTEX_LANGUAGE not supported."
            )
    ...
    def format_language_list(self) -> str:
        ...
        """
        names = [
            self.SUPPORTED_LANGUAGES[code] for code in sorted(self.SUPPORTED_LANGUAGES)
        ]
        return ", ".join(names)
